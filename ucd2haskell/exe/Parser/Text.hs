...
genBlocksModule moduleName = done <$> Fold.foldl' step initial
    where
        ...
        , "-- | Block definition"
        , "--"
        , "-- Undefined for values greater than " <> show (count - 2) <> "."
        , "--"
        , "-- Encoding:"
        , "--"
    ...
    encodeUtf32 :: Int -> String
    encodeUtf32 = foldMap (("\\" <>) . show) . word32ToWord8s . fromIntegral
...
genNamesModule moduleName =
    ...
    encode32LE v acc
        ...
        , "    ( Addr#, Char#, Ptr(..),"
        , "      ord#, (+#), (-#), (<#), (>#), (==#),"
        , "      uncheckedIShiftL#, uncheckedIShiftRL#,"
        , "      plusAddr#, isTrue# )"
        , "import Unicode.Internal.Bits.Names (lookupInt32#, SPEC(..))"
        , ""
        ...
        , "{-# INLINE name #-}"
        , "name :: Char# -> Addr#"
        , "name c# = getName SPEC 0# " <> shows (length names - 1) "#"
        , "    where"
        , "    -- [NOTE] Encoding"
